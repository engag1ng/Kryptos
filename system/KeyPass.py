...
def gak():
    my_image = ctk.CTkImage(dark_image=Image.open("hotp.png"), light_image=Image.open("hotp.png"), size=(150,150))
...
try: #On first startup...
    ...
    qrcode.make(uri).save("hotp.png") #Create QR-Code from URL and save it to 'hotp.png'
    first = ctk.CTk()
    first.title("Setup")
...
def ROOT():
    ...
    Search Password:
    When clicking the 'Search Password' Button a new window opens, which asks for 2 inputs. You can input maybe a few digits of the Password if you remember and in the second field a Username or Email that you previously assigned to any passwords. So for example you saved a password '1234' with the Username 'MyUsernameForGoogle'. If you now want to search the password you can just type 'MyUsernameForGoogle' in the Searchbar for Username and Email, press search and a list will display all of your Passwords with the Username/Email 'MyUsernameForGoogle' assigned to it. Remember the Searchbar is case-sensitive, that means that 'myusernameforgoogle' wouldn't provide any results.

    New Password:
    If you want to search you first have to create. That's where the New Password Button comes in. When clicking it a new window opens. The first entry field is for the password you want to save. As the Warning on the Start screen says. It is recommended to use a generated password. You can easily generate one with the Generate Button. The Username/Email field is pretty self-explanatory. Just put in a Username or Email you want to associate with the password. As an example Password: '<b$W'$pfoRkL' and Username/Email: 'MyUsernameForGoogle'. If you want to get fancy you can also put something like 'MyUsernameForGoogle ; MyEmail' in the Username/Email field so you can search for both Username AND Email.
